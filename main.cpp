...
int main( int argc, char** argv )
{
    if( argc != 2)
    {
        cout <<" Usage: ./ProjetSI343 ImageToLoadAndDisplay" << endl;
        return -1;
    }
    ...
    /*
     ...
     //if (i==2256) {cout <<  j << std::endl ;}
     gradSP.at<Vec3b>(i,j)[0]=sqrt(abs_grad_x.at<Vec3b>(i,j)[0]^2+abs_grad_y.at<Vec3b>(i,j)[0]^2);
     gradSP.at<Vec3b>(i,j)[1]=sqrt(abs_grad_x.at<Vec3b>(i,j)[0]^2+abs_grad_y.at<Vec3b>(i,j)[0]^2);
     gradSP.at<Vec3b>(i,j)[2]=sqrt(abs_grad_x.at<Vec3b>(i,j)[0]^2+abs_grad_y.at<Vec3b>(i,j)[0]^2);
    ...
    /*vector<Vec2f> lines;
     HoughLines(gradS, lines, 1, CV_PI/90, 200, 0, 0 );
     for( size_t i = 0; i < lines.size(); i++ )
     {
     ...
     Mat houghLineTemp=Mat(gradS.size(),CV_8UC3);
     line( houghTrans, pt1, pt2, Scalar(0,0,255), 1, LINE_AA);
     line( houghLineTemp, pt1, pt2, Scalar(5,5,5), 1, LINE_AA);
     add(houghLineTemp, houghLine, houghLine);
     }*/
    ...
    for (int i=0; i<lines.size(); i++) {
        for (int j=i+1; j<lines.size(); j++) {
            ...
            if (fmod (abs(lines[i][1]-lines[j][1]),2*pi)>0.05 /*environ 50 deg */) {
                ...
                //cout <<"theta diff: " <<lines[i][1]<< "theta1: "<<lines[j][1] <<std::endl;
                //cout <<"theta diff: "<<fmod (abs(lines[i][1]-lines[j][1]),2*pi) <<std::endl;
    ...
    std::cout << "nombre coin seul : "<< test << std::endl;
    ...
    //imshow( "Display window", houghTrans );
    /* resize(gradS,gradS2 ,Size(gradS.cols/7, gradS.rows/7),0, 0, INTER_AREA);
     imshow( "Display window", gradS2 );
     
     
     //affichage Harris
     namedWindow( "Display Harris", WINDOW_AUTOSIZE );
     resize(HarrisRes,HarrisRes ,Size(gradS.cols/7, gradS.rows/7),0, 0, INTER_AREA);
     imshow( "Display Harris", 1000*HarrisRes );
     
     Mat visu;
     
     cvtColor( gradS, gradS, cv::COLOR_GRAY2RGB );
     add(image, houghLine , visu);
     */
    
    
    /*resizeWindow("Display Corner", image.cols/7, image.rows/7);
     resizeWindow("Display window", image.cols/7, image.rows/7);*/
